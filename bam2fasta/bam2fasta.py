...
def collect_reduce_temp_fastas(index, umi_filter):
...
def unfiltered_umi_to_fasta(
    ...
    for fasta in iter_split(single_barcode_fastas, ","):
        ...
        if count == 0:
            ...
            f = open(os.path.join(save_fastas, unique_fasta_file), "w")
...
def bam_to_fasta(
    ...
    if barcodes_file is not None:
        barcodes = tenx_utils.read_barcodes_file(barcodes_file)
    else:
        barcodes = None

    # Shard bam file to smaller bam file
    print('... reading bam file from {}'.format(
        filename))
    n_jobs = processes
    filenames, mmap_file = np_utils.to_memmap(np.array(
        tenx_utils.shard_bam_file(filename, line_count, os.getcwd())))
    ...
    del all_fastas
    print("Created fasta_files_dict")
